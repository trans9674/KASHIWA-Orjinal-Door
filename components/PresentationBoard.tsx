                  ...
                  <>
                    {items.map((item, idx) => {
                      if (item.type === 'door') {
                        ...
                        return (
                          <div key={`door-${door.id}`} className="bg-gray-50 rounded-lg overflow-hidden border border-black flex flex-col h-full min-h-[130px]">
                            {/* Image Area */}
                        ...
                        return (
                          <div key="storage-item" className="bg-gray-50 rounded-lg overflow-hidden border border-black flex flex-col h-full min-h-[130px]">
                        ...
                        return (
                          <div key={`baseboard-${idx}`} className="bg-gray-50 rounded-lg overflow-hidden border border-black flex flex-col h-full min-h-[130px]">
                            <div className="h-24 bg-white p-1 relative flex items-center justify-center border-b border-black">
                              {record?.pbImageUrl ? (
                        ...
                        return (
                          <div key={`handle-${idx}`} className="bg-gray-50 rounded-lg overflow-hidden border border-black flex flex-col h-full min-h-[130px]">
                            <div className="h-24 bg-white p-1 relative flex items-center justify-center border-b border-black">
                              {record?.pbImageUrl ? (
                                <img src={record.pbImageUrl} className="max-h-full max-w-full object-contain" />
                              ) : (
                    ...
                    {Array.from({ length: Math.max(0, 24 - items.length) }).map((_, i) => (
                      <div key={`empty-${i}`} className="bg-gray-50/30 rounded-lg border border-dashed border-black min-h-[130px]"></div>
                    ))}
                  </>
                );
              })()}
