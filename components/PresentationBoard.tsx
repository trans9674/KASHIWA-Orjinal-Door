...
import { DoorItem, OrderState, PriceRecord } from '../types';
...
export const PresentationBoard: React.FC<PresentationBoardProps> = ({ order, priceList, storageTypes, onClose }) => {
  return (
    <div className="fixed inset-0 z-[600] bg-gray-900/90 backdrop-blur-md overflow-y-auto p-4 md:p-8 no-print">
      <div className="max-w-[1200px] mx-auto relative">
        ...
        <div className="bg-white shadow-2xl mx-auto print:shadow-none print:m-0 overflow-hidden rounded-sm" 
             style={{ width: '100%', maxWidth: '297mm', minHeight: '420mm', margin: '0 auto' }}>
          
          {/* Header Area */}
          <div className="bg-slate-800 text-white p-8 mb-8 flex justify-between items-end">
            <div>
                      ...
                      <div key={door.id} className="bg-slate-50 rounded-xl overflow-hidden border border-slate-200 flex h-52">
                        ...
                        <div className="w-2/3 p-4 flex flex-col justify-between">
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-y-1 text-[10px] text-slate-600">
                              ...
                              <div className="flex items-center gap-1">
                                <div className="w-2 h-2 rounded-full border border-slate-300" style={{ backgroundColor: COLOR_MAP[door.doorColor] || '#ccc' }}></div>
                                {door.doorColor}
                              </div>
                              <div className="font-bold text-slate-400">カラー（枠）</div>
                              <div className="flex items-center gap-1">
                                <div className="w-2 h-2 rounded-full border border-slate-300" style={{ backgroundColor: COLOR_MAP[door.frameColor] || '#ccc' }}></div>
                                {door.frameColor}
                              </div>
                          ...
                          <div className="pt-2 border-t border-slate-200 flex justify-between items-center">
                            <span className="text-[9px] text-slate-400 font-bold uppercase">Kashiwa-Mokko Original Door</span>
                      ...
                      <div key={idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex flex-col justify-between">
                         <div>
                            <div className="flex items-center gap-2 mb-2">
                               <div className="w-4 h-4 rounded-full border border-slate-300" style={{ backgroundColor: COLOR_MAP[item.color] || '#ccc' }}></div>
